...
import Selection
import caretRangeFromPoint
import element
import getSelection
import kotlinx.browser.document
import kotlinx.browser.window
import markdown.MarkdownComponent
import markdown.MarkdownElementEscapeByBackspace
import markdown.MarkdownElementWithLevel
import markdown.ZERO_WIDTH_SPACE
import markdown.components.MarkdownImage
import markdown.components.MarkdownLink
import markdown.normalizeUnknownChild
...
abstract class BaseMarkdownDocument : BaseWebComponent() {
    ...
    private fun firstUpdated() {
        if (getAttribute("floating-toc") == "true") {
            val toc = document.createElement("markdown-toc") as markdown.components.MarkdownToc
    ...
    fun getCurrentLeafBlock(): markdown.LeafElement? {
        var element: Node? = getSelection()?.anchorNode
        while (element != null && element != document && element != this) {
            if (element is markdown.LeafElement) return element
            element = element.parentNode
        }
        return null
    }

    fun getLastLeafBlock(): markdown.LeafElement? {
...
fun surroundRangeIfNotYet(tagName: String, range: Range) {
    ...
    getAllTextNodesForRange(range).forEach { text ->
        if (text.parentElement?.closest(tagName) == null) {
            ...
            while (replaceLevel.parentElement is markdown.TerminalInlineElement) {
